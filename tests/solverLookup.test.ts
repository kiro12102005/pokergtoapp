...
describe("solverLookup - both game formats", () => {
  it("has a precomputed rfi situation for every open position, at a representative depth in each format", () => {
    for (const format of FORMATS) {
      for (const position of ["UTG", "HJ", "CO", "BTN", "SB"] as const) {
        expect(hasPreflopSituation(position, 100, "rfi", format)).toBe(true);
      }
    }
  });

  it("cash supports deeper stack depths than tournament ever solves (150BB/200BB)", () => {
    expect(hasPreflopSituation("BTN", 200, "rfi", "cash")).toBe(true);
    ...
    expect(hasPreflopSituation("BTN", 200, "rfi", "tournament")).toBe(true);
  });

  it("frequencies sum to ~1 for a sample of hands in both formats", () => {
    for (const format of FORMATS) {
      for (const hand of ["AA", "72o", "KQs"]) {
        const mix = lookupPreflopStrategy("BTN", 100, "rfi", hand, format);
  ...
  it("cash and tournament produce meaningfully different shove ranges at short stacks", () => {
    ...
    for (const hand of ALL_HANDS) {
      const t = lookupPreflopStrategy("UTG", 7, "rfi", hand, "tournament");
      const c = lookupPreflopStrategy("UTG", 7, "rfi", hand, "cash");
  ...
  it("hasPreflopSituation is false for a situation that was never solved (BB never opens)", () => {
    // OPEN_POSITIONS in precompute-preflop.ts excludes BB - it's never the position that RFIs.
    for (const format of FORMATS) {
      expect(hasPreflopSituation("BB", 100, "rfi", format)).toBe(false);
    }
  });

  it("lookupPreflopStrategy throws SolverLookupError for that same never-solved situation", () => {
    expect(() => lookupPreflopStrategy("BB", 100, "rfi", "AA", "tournament")).toThrow();
  });

  it("an out-of-range stack depth clamps to the nearest solved bucket rather than erroring", () => {
    expect(hasPreflopSituation("BTN", 999999, "vs4bet", "cash")).toBe(true);
  });

  it("higher rake monotonically tightens a marginal calling range (cash only)", () => {
    ...
    const callWidth = (rakePercent: 0 | 0.05 | 0.1) => {
      let count = 0;
      for (const hand of ALL_HANDS) {
        const mix = lookupPreflopStrategy("SB", 15, "vsOpen", hand, "cash", rakePercent);
        if ((mix.call ?? 0) > 0.5) count++;
      }
      return count;
    };
    const noRake = callWidth(0);
    const fivePercent = callWidth(0.05);
    const tenPercent = callWidth(0.1);
  ...
  it("tournament lookups ignore any rake argument (rake is meaningless outside cash)", () => {
    const a = lookupPreflopStrategy("BTN", 100, "rfi", "AA", "tournament");
    const b = lookupPreflopStrategy("BTN", 100, "rfi", "AA", "tournament", 0.1);
    expect(a).toEqual(b);
  });
});
