...
describe("buildHandRecordSnapshot", () => {
  it("returns null when hero's hand isn't fully picked", () => {
    expect(buildHandRecordSnapshot({ ...baseDraft, heroCards: [null, null] }, "tournament")).toBeNull();
    expect(buildHandRecordSnapshot({ ...baseDraft, heroCards: [card(13, "s"), null] }, "tournament")).toBeNull();
  });

  it("filters unset board slots down to only the picked cards", () => {
    const snapshot = buildHandRecordSnapshot({ ...baseDraft, board: [card(14, "s"), null, null] }, "tournament");
    expect(snapshot?.board).toEqual([card(14, "s")]);
  });

  it("truncates action history to streets up to and including the current one", () => {
    const snapshot = buildHandRecordSnapshot(baseDraft, "tournament");
  ...
  it("trims blank extra context down to undefined", () => {
    const snapshot = buildHandRecordSnapshot({ ...baseDraft, extraContext: "   " }, "tournament");
    expect(snapshot?.extraContext).toBeUndefined();
  });

  it("keeps non-blank extra context", () => {
    const snapshot = buildHandRecordSnapshot({ ...baseDraft, extraContext: " 相手はタイト " }, "tournament");
    expect(snapshot?.extraContext).toBe("相手はタイト");
  });

  it("stamps the snapshot with the given format", () => {
    expect(buildHandRecordSnapshot(baseDraft, "cash")?.format).toBe("cash");
    expect(buildHandRecordSnapshot(baseDraft, "tournament")?.format).toBe("tournament");
  });
});
