...
const SECTIONS: HelpSection[] = [
  {
    ...
    body: "「GTO的視点」は、ポットオッズから計算した必要勝率と、想定レンジに対するヒーローの実際のエクイティを、AIを使わずこのアプリが計算した値です。「AIによるエクスプロイト視点」はそのGTOの数値を土台にしつつ、クラブマッチ/リングキャッシュでよくある相手の傾向(コーリングステーション寄り、リバーのブラフ頻度が低い等)を踏まえてAIが提案する調整後の頻度です。どちらも参考値であり、AI側は正式なGTOソルバーの厳密解ではありません。",
  },
  {
    title: "有効スタックとポット",
  ...
  {
    title: "分析モードの使い方",
    body: "画面上部でクラブマッチ(トーナメント)/リングキャッシュ(レーキ設定つき)を切り替えられます。実際にプレイしたハンドの状況(ポジション・スタック・アクション履歴・ボード)を入力すると、プリフロップは事前計算されたソルバーの厳密解、ポストフロップ(またはクラブマッチで相手のスタック情報を入力したICM絡みの状況)はAIによる推奨を確認できます。ログインすると結果を履歴に保存でき、各ハンドは「共有」をONにするとログイン不要の読み取り専用リンクを発行できます。履歴ページからは保存済みの全件をJSON/CSVでも書き出せます。",
  },
  {
    title: "練習モードの使い方",
    body: "画面上部でクラブマッチ/リングキャッシュ(レーキ設定つき)を切り替えられます。「プリフロップ」タブは事前計算された厳密解に基づく練習、「ポストフロップ」タブはフロップ・ターン・リバーいずれかの局面(GTOベースライン+AIのエクスプロイト評価)を練習できます。ポストフロップの絞り込み(局面の種類・ストリート・ポジション)を使うと、特定の状況だけを集中して練習できます。",
  },
  {
