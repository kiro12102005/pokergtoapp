...
export function Seat({ position, stackBB, isHero, isToAct, lastAction, style }: SeatProps) {
  ...
  return (
    <div
      style={style}
      className={`absolute flex w-20 -translate-x-1/2 -translate-y-1/2 flex-col items-center gap-0.5 rounded-lg border px-2 py-1 text-center transition-opacity ${
        isHero
    ...
    >
      <div className="flex items-center gap-1 text-xs font-bold text-zinc-700 dark:text-zinc-200">
        {position === "BTN" && (
          <span className="flex h-4 w-4 items-center justify-center rounded-full bg-zinc-800 text-[10px] text-white dark:bg-zinc-200 dark:text-zinc-900">
            D
          </span>
        )}
        {position}
